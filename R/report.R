...
report <- function(
  ...
  if (!is.null(simulation.path)) {
    ...
    if ('results' %in% ls()) {
      #'results' is the name of an environment containing an incomplete simulation
      ...
      if (r.resume <= env_get(results, 'n.sim')) {
        ...
        output <- simulate(
          ...
          optimize.cgm = optimize.cgm,
          compare.to.vdg = compare.to.vdg,
      ...
      env_bind(output, simulation.path = simulation.path)
    }
  } else {
    if (!is.null(inherit.data)) {
      if (!str_detect(inherit.data, '.RData$')) {
        stop(
          "inherit.data argument must be a .RData file containing data to use in the simulation."
        )
      }
      load(inherit.data)
      env_coalesce(current_env(), output)
      message(
        'Inheriting the data used for the simulations in ',
        inherit.data,
        "."
      )
      rm(output)
  ...
  if (file.exists(folder) & sim.active) {
    ...
    paste0(
      "Simulation results for these parameters and methods already exist. Saving results to folder ",
      location,
      "."
    ) |>
      message()
  ...
  env_bind(
    output,
    date = Sys.Date(),
    simulation.path = simulation.path,
    note = note,
    ec = ec,
    folder = folder,
