...
decompose <- function(A, A.build, y, X, lattice){
    n <- ifelse(length(unique(dim(A)))==1, unique(dim(A)), stop("Adjacency matrix must be a square matrix."))
    n.sim <- ncol(y)
  ...
  # will need to add arguments to get this part to work
   ...
   return(list(A.ordered=A.ordered, y.ordered=y.ordered, X.ordered=X.ordered, mvc=mvc))
}
