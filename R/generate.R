...
generate <- function(
  ...
  if (lattice) {
    ...
    message(paste0(
      "Using rectangular lattice with ",
      factors[1],
      " rows and ",
      factors[2],
      " columns."
    ...
    lattice.points <- cbind(
      point = 1:n,
      row = 1:factors[1],
      column = rep(1:factors[2], times = rep(factors[1], n / factors[1]))
    )
    ...
    for (i in 1:n) {
      ...
      distances <- abs(
        lattice.points[, 2:3] - matrix(rep(position, n), ncol = 2, byrow = TRUE)
      )
  ...
  X <- array(runif(n * d * n.sim, min = -k, max = k), dim = c(n, d, n.sim))
  ...
  predictor <- apply(X, 3, '%*%', theta)

  ## Gibbs Sampler
  if (keep.every %% 1 > 0) {
    keep.every <- ceiling(keep.every)
    message(paste0(
      "keep.every must be an integer. generate() increased keep.every to ",
      keep.every,
      "."
    ))
  }
  ...
  #   # Original sampling algorithm---Gibbs sampling incorrect here
  #    m <- A %*% y
  #    X.temp <- matrix(runif(n*d,min=-k,max=k),nrow=n,ncol=d)
  #    predictor.temp <- X.temp %*% theta
  #    trans_pr <- exp(predictor.temp+beta*m)/(exp(predictor.temp+beta*m)+exp(-predictor.temp-beta*m))
  #    y <- 2*rbinom(n, 1, trans_pr)-1
  #   # indices.used <- c()
  #   # for(i in 1:n){
  ...
  return(list(
    ...
    beta = beta,
    A = A,
    A.build = if (lattice) position.matrix else NULL,
    probs = trans_pr
  ))
}
