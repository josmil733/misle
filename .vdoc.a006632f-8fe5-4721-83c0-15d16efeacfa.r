...
#
# cat(theta, ",")
cat(paste0('$\\theta[', indices.on, '] = ', round(theta[indices.on],3), "$ \\hspace{3pt}", collapse=""))
#
#
#
kable(data.frame(Parameter=indices.on, True.Value=round(theta[indices.on],3)), caption="True Nonzero Parameter Values")
#
#
#
#
...
#
#
cat("### Mean-squared error comparison $(\\frac{1}{n.sim}\\sum_{i=1}^{n.sim} \\frac{1}{|\\mathcal C|}\\|\\hat\\theta_{i,\\mathcal C}-\\theta_{\\mathcal C}\\|^2)$")
...
#
#
cat("### Mean absolute deviation comparison $(\\frac{1}{n.sim}\\sum_{i=1}^{n.sim} \\frac{1}{|\\mathcal C|}\\|\\hat\\theta_{i,\\mathcal C}-\\theta_{\\mathcal C}\\|)$")

mad <- matrix(NA, nrow=length(covts.record)+1, ncol=length(results.env), dimnames = list(c(paste0("theta[", covts.record, "]"), 
"total"), methods.detected))
...
for(m in methods.detected){
  result = env_get(results.env, paste0("results.hist.", m) )
  mad[-nrow(mad),m] <- abs(result$value-matrix(theta[covts.record], nrow=length(covts.record), ncol=n.sim)) |> rowMeans()
  mad[nrow(mad),m] <- mean(mad[-nrow(mad),m])
  if(m != 'vdg'){
    mad.first.step[-nrow(mad),m] <- abs(result$first.step-matrix(theta[covts.record], nrow=length(covts.record), ncol=n.sim)) |> rowMeans()
    mad.first.step[nrow(mad),m] <- mean(mad.first.step[-nrow(mad),m])
  }
}

kable(mad, digits=3, caption="Mean Absolute Deviation of Parameter Estimates")
kable(mad.first.step, digits=3, caption="Mean Absolute Deviation of First-Step Parameter Estimates")
#
#
#
#
...
for(i in covts.record){
  for(m in methods.detected){
    result = env_get(results.env, paste0("results.hist.", m) )
    hist(result$value[result$parameter.no==i,], freq = FALSE, main=paste0("Histogram of ", m, " estimates for theta[", i, "]=", theta[i] |> round(3)), cex.main=0.6, xlab=NULL)
...
for(i in covts.record){
  for(m in methods.detected){
    result = env_get(results.env, paste0("results.hist.", m) )
    hist(result$first.step, freq = FALSE, main=paste0("Histogram of ", m, " first-step estimates for theta[", i, "]=", theta[i] |> round(3)), cex.main=0.5, xlab=NULL)
    abline(v=theta[i], col='green', lty='dashed', lwd=2.5)
  }
}
#
