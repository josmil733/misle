...
if(is.null(r.resume)){
  ...
  if(sample.split){
    ...
    A.m <- A.m[mvc, mvc.c] #1/29: should cause an error
      ...
      sqe <- matrix(-1, nrow=n.sim, ncol=d, dimnames=list(rep=paste0(1:n.sim), parameter=paste0("theta[", 1:d, "]")) )

      if(compare.to.cgm) sqe.cgm <- matrix(-1, nrow=n.sim, ncol=d, dimnames=list(rep=paste0(1:n.sim), parameter=paste0("theta[", 1:d, "]")) )
  ...
  if(!r%%min(5, floor(n.sim/5))){
    ...
    if(r==min(5, floor(n.sim/5))){
      paste0("Saving data after ", r, " replications to ", getwd(), "/", file.name)
    }
