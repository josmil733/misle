...
This simulation is performed with $n=`r n`$ and $d=`r d`$, using the 2-d lattice as the underlying graph. $s=`r s`$ parameters are set to be nonzero, and the beta parameter is chosen to be $\beta=`r beta`$. The attached results are for a `r n.sim`-replication simulation. The parameter vector $\theta$ has sparse components other than the following:
...
```{r}
# kable(data.frame("Parameter Index"=indices.on, Value=round(theta[indices.on],3)), caption="True Nonzero Parameter Values")

df_k <- data.frame("Parameter Index" = indices.on, Value = round(theta[indices.on], 3))
kable(df_k, format = "latex", booktabs = FALSE) |>
  kable_styling(latex_options = "hold_position") #|>
  # column_spec(1:ncol(df_k), border_left = TRUE, border_right = TRUE) |>
  # row_spec(0, hline_after = TRUE) |>
  # row_spec(1:nrow(df_k), hline_after = TRUE)
```
...
```{r, results='asis', eval=params$boxplot}
cat("### Boxplots")
```
